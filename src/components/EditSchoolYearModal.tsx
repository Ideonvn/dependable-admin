...
export default function EditSchoolYearModal({
  isOpen,
  onClose,
  onSuccess,
  schoolId,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await schoolsApi.updateSchoolYear(schoolId, year.id, {
        name: name.trim(),
        starts_on: startsOn,
        ends_on: endsOn,
      });
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to update school year';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 dark:bg-opacity-75 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-white dark:bg-[#121212] rounded-lg shadow-xl border border-gray-200 dark:border-gray-800 max-w-md w-full my-8">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-800">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-gray-100">Edit School Year</h2>
          <button
            onClick={onClose}
            disabled={submitting}
