...
export default function ContactProfileImage({
  ...
  useEffect(() => {
    if (!imageFilename || !schoolId || !studentId || !contactId) {
      setIsLoading(false);
      return;
    }
    ...
    const fetchImage = async () => {
      try {
        ...
        const response = await fetch(
          `${apiUrl}/schools/${schoolId}/students/${studentId}/contacts/${contactId}/profile/image/${imageFilename}`,
          {
            headers: {
        ...
        console.error('Error fetching contact profile image:', err);
        setError(true);
      } finally {
        setIsLoading(false);
      }
  ...
  }, [schoolId, studentId, contactId, imageFilename]);
