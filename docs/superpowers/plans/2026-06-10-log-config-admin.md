# Log Config Admin UI Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a super-admin-only "Logging" section to the dashboard that lets admins manage log fields and their selectable options via the `/admin/log-config` API.

**Architecture:** Mirror the existing System Users pattern — a server page at `/system/log-config` that gates on super-admin, rendering a client component. The client fetches all fields (grouped by log_source into tabs), then lazily loads options for each `tile-select` field when its tab is active. A new `src/lib/logConfig.ts` module owns all API calls.

**Tech Stack:** Next.js 16 App Router, React 19, TypeScript, Axios (via `src/lib/api.ts`), Tailwind CSS 4, Lucide icons.
...
| File | Action | Purpose |
|------|--------|---------|
| `src/lib/logConfig.ts` | Create | All `/admin/log-config` API calls + TypeScript types |
| `src/app/system/log-config/page.tsx` | Create | Server page — super-admin gate + renders client |
| `src/components/system/LogConfigClient.tsx` | Create | Full interactive client: tabs, fields table, options management |
| `src/components/DashboardClient.tsx` | Modify | Add "Logging" button next to System Users |

---

### Task 1: API module — `src/lib/logConfig.ts`

**Files:**
- Create: `src/lib/logConfig.ts`
...
```bash
cd /Users/ideon/src/dependable-admin && npm run lint -- --max-warnings=0 2>&1 | tail -20
```
...
```bash
git add src/lib/logConfig.ts
git commit -m "feat: add logConfig API module with types"
```

---

### Task 2: Server page — `/system/log-config`

**Files:**
- Create: `src/app/system/log-config/page.tsx`

- [ ] **Step 1: Create the page (mirrors `/system/users/page.tsx`)**
...
> Note: `isSuperAdminEmail()` reads `userSetupService` which is client-side. The redirect here is a server-side safety belt; the dashboard button is also gated. This matches the existing pattern in `src/app/system/users/page.tsx` — the server page checks `session` exists, and the client component re-checks admin status.

Actually, looking at `src/app/system/users/page.tsx` it does NOT call `isSuperAdminEmail()` server-side (that helper reads localStorage). The gate is purely done by the dashboard hiding the button + `src/proxy.ts` middleware. Match that pattern exactly:
...
```bash
git add src/app/system/log-config/page.tsx
git commit -m "feat: add log-config server page"
```
...
- [ ] **Step 1: Add the Logging button import and button JSX**

In `src/components/DashboardClient.tsx`, change the import line for lucide icons from:
```typescript
...
Then add a new button after the existing System Users button (inside the `{isSuperAdmin && ...}` block, before the Onboarding button):
...
git commit -m "feat: add Logging button to dashboard (super-admin only)"
```

---

### Task 4: `LogConfigClient` — scaffold + fields list

**Files:**
- Create: `src/components/system/LogConfigClient.tsx`

This task creates the full component: header, source tabs, fields table with inline edit, options section, and all modals. It is split into logical sub-steps but all in one file.

- [ ] **Step 1: Create the component with fields loading and tab structure**
...
export default function LogConfigClient() {
  ...
  // Edit-option inline state: optionId -> draft label
  ...
  const loadFields = useCallback(async () => {
    ...
    try {
      ...
      setFields(data);
      if (!activeSource && data.length > 0) {
        setActiveSource(data[0].log_source);
      }
    } catch {
  ...
  const loadOptions = useCallback(
    async (field: LogField) => {
      ...
      if (optionsMap[key] !== undefined || optionsLoading[key]) return;
      setOptionsLoading((c) => ({ ...c, [key]: true }));
      try {
        const data = await logConfigApi.listOptions(field.log_source, field.field_name);
    ...
    [optionsMap, optionsLoading, addToast]
  );
  ...
  const activeFields = activeSource ? (grouped[activeSource] ?? []).slice().sort((a, b) => a.sort_order - b.sort_order) : [];
    ...
    <main className="w-full flex-1 px-4 sm:px-6 lg:px-8 py-8 bg-gray-50 dark:bg-[#0F1115]">
                ...
                <tbody className="divide-y divide-gray-200 dark:divide-gray-800">
                  {activeFields.map((field) => (
                    <>
                      ...
                      {/* Options sub-row for tile-select fields */}
      ...
      {editingField && (
        <Modal title="Edit Field" onClose={() => setEditingField(null)}>
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={() => setEditingField(null)} disabled={editSaving} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button onClick={saveEditField} disabled={editSaving} className="px-4 py-2 text-sm font-medium text-white bg-[#1A1A6D] dark:bg-[#20B2AA] rounded-lg hover:opacity-90 transition-colors disabled:opacity-50 flex items-center gap-2">
      ...
      {addFieldOpen && activeSource && (
        <Modal title={`Add Field — ${activeSource}`} onClose={() => setAddFieldOpen(false)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Field Name <span className="text-xs text-gray-500">(snake_case)</span></label>
              <input
            ...
            <div className="flex items-center gap-4 mt-7">
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                <input type="checkbox" checked={addFieldForm.is_required} onChange={(e) => setAddFieldForm((c) => ({ ...c, is_required: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 dark:border-gray-700" />
                Required
              </label>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                <input type="checkbox" checked={addFieldForm.is_hidden} onChange={(e) => setAddFieldForm((c) => ({ ...c, is_hidden: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 dark:border-gray-700" />
                Hidden
              </label>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
                <input type="checkbox" checked={addFieldForm.is_source_attribute} onChange={(e) => setAddFieldForm((c) => ({ ...c, is_source_attribute: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 dark:border-gray-700" />
                Source Attribute
              </label>
            </div>
          </div>
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={() => setAddFieldOpen(false)} disabled={addFieldSaving} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button
      ...
      {addOptionField && (
        <Modal title={`Add Option — ${addOptionField.label}`} onClose={() => setAddOptionField(null)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Label <span className="text-xs text-gray-500">(defaults to value)</span></label>
              <input
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={() => setAddOptionField(null)} disabled={addOptionSaving} className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button
        ...
        {toasts.map((t) => (
          <div
            ...
            className={`max-w-sm w-full px-4 py-3 rounded-lg shadow-lg border bg-white dark:bg-[#111217] border-gray-200 dark:border-gray-800 ${
              t.variant === 'success' ? 'ring-2 ring-green-500 dark:ring-green-400' : 'ring-2 ring-red-500 dark:ring-red-400'
            }`}
          >
            ...
            >
              {editingOptionId === opt.id ? (
                <>
                  ...
                  />
                  <button onClick={() => onEditSave(opt)} disabled={optionSaving} className="text-green-600 dark:text-green-400 hover:text-green-800 disabled:opacity-50">
                ...
                <>
                  ...
                  <button onClick={() => onEditStart(opt)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 ml-1" title="Edit label">
                    <Pencil className="w-3 h-3" />
                  </button>
                  <button onClick={() => onToggleHide(opt)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200" title={opt.is_hidden ? 'Show' : 'Hide'}>
                    {opt.is_hidden ? <Eye className="w-3 h-3" /> : <EyeOff className="w-3 h-3" />}
                  </button>
                  <button onClick={() => onDelete(opt)} className="text-red-400 hover:text-red-600" title="Delete">
...
```bash
cd /Users/ideon/src/dependable-admin && npm run lint -- --max-warnings=0 2>&1 | tail -30
```

Expected: no new errors. Fix any TypeScript issues (e.g. missing key on fragment — replace `<>` with `<React.Fragment key={...}>`).

- [ ] **Step 3: Build check**

```bash
cd /Users/ideon/src/dependable-admin && npm run build 2>&1 | tail -30
```
...
```bash
git add src/components/system/LogConfigClient.tsx
git commit -m "feat: add LogConfigClient component with fields/options management"
```
...
- [ ] **Step 1: Start dev server and smoke-test manually**

```bash
cd /Users/ideon/src/dependable-admin && npm run dev
```

Navigate to `http://localhost:3000` — confirm "Logging" button appears (when logged in as super-admin).
Click it — confirm the Log Config page loads, tabs appear, fields table renders, options chips appear under `tile-select` fields.

- [ ] **Step 2: Final lint + build**

```bash
cd /Users/ideon/src/dependable-admin && npm run lint && npm run build 2>&1 | tail -20
```
...
```bash
git add -A
git commit -m "feat: log config admin UI — fields and options management"
```
